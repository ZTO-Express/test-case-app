<template>
  <div class="env-msg-gloaba">
    <div>
      <el-form ref="search" inline label-width="80px" style="margin-bottom: 20px">
        <span>选择模块</span>
      </el-form>
    </div>
    <div class="flex h100" id="treeinfos" style="height: 500px;overflow-x: auto">
      <div class="uscMsg-tree-index flex-1" >
        <el-input placeholder="输入关键字进行过滤" v-model="filterText" clearable style="margin-bottom: 8px;"/>
        <el-tree :data="treeData" :props="defaultProps" node-key="id" :default-expanded-keys="openkeys" :filter-node-method="filterNode"  ref="departmentTree" >
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      this.$confirm('确认移动所选用例到[' + this.moduleName + ']模块下?', {
