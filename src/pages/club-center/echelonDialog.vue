<template>
  <div class="echelon-center-dialog">
    <el-dialog
      :title="title"
      ...
      <el-form :model="dialogForm" :rules="dialogForm1Rules" ref="dialogForm" label-width="120px" size="small">
        <el-form-item prop="echelonName" label="梯队名称">
          <el-input style="width: 360px" v-model.trim="dialogForm.echelonName" clearable maxlength=100
                    placeholder="梯队名称"/>
        </el-form-item>
        <el-form-item prop="echelonAgeStart" label="年龄段">
          <el-input style="width: 175px" v-model.trim="dialogForm.echelonAgeStart" clearable maxlength=2>
            <template slot="append">岁</template>
          </el-input>
          -
          <el-input style="width: 175px" v-model.trim="dialogForm.echelonAgeEnd" clearable maxlength=2>
            <template slot="append">岁</template>
          </el-input>
        </el-form-item>
      </el-form>
    ...
    methods: {
      ...
      async submit() {
        this.$refs.dialogForm.validate((pass) => {
          ...
          let reqUrl = '';
          if (this.curType === 'new') {
            reqUrl = this.$urlConst.ADD_ECHELON;
          }
          if (this.curType === 'edit') {
            reqUrl = this.$urlConst.EDIT_ECHELON;
          }
...
<style>
  /*间距*/
  .echelon-center-dialog .el-dialog {
    width: 550px !important;
  }

  .echelon-center-dialog h4 {
    color: #888;
    font-weight: normal;
  }

  .echelon-center-dialog .el-dialog__body {
    overflow: hidden;
    padding: 20px 30px 30px !important;
  }

  .echelon-center-dialog .el-input.is-disabled .el-input__inner {
    cursor: default; /*输入框禁用鼠标样式为默认*/
    color: #000;
  }

  .echelon-center-dialog
  .el-select
  ...
  .echelon-center-dialog .el-input.is-disabled .el-input__icon {
