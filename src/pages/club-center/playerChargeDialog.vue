  ...
  <div class="player-charge-dialog">
    <el-dialog
      ...
      <el-form :model="dialogForm" :rules="dialogForm1Rules" ref="dialogForm"
        ...
        <el-form-item prop="auditWay" label="支付方式">
          <el-select v-model.trim="dialogForm.auditWay" clearable placeholder="支付方式">
            <el-option v-for="item in auditWayArr" :key="item.value" :label="item.label"
                       :value="item.value"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item prop="seqId" label="支付凭证">
          <el-input v-model="dialogForm.seqId" maxlength="32">
          </el-input>
        </el-form-item>
    ...
    methods: {
      ...
      async submit() {
        this.$refs.dialogForm.validate((pass) => {
          ...
          this.$axiosUtil.post(this.$appConfig.MIX, this.$urlConst.PLAYER_CHARGE, params).then((res) => {
            if (res.code === 'S0001') {
              this.loading = !this.loading;
              this.close();
