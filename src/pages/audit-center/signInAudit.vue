<template>
  <div>
    ...
    <el-row>
      <signIn-audit-dialog :visible.sync="dialogVisible" :dialogType="dialogType"
      ...
      <el-card class="table-box gap" align="center">
        <el-table :data="data.viewData.list" border v-loading="loading"
          ...
          <el-table-column align="center" width='80px' fixed="right" label="操作">
            <template slot-scope="scope">
              <el-button type="primary" size="mini" @click="openAuditInfo(scope.row, scope.$index)"
                         v-permission="'audit-center/signInAudit'"
...
<script>
  ...
  import SignInAuditDialog from "./signInAuditDialog";
  ...
  export default {
    ...
    methods: {
      ...
      openAuditInfo(row = {}, index = '',) {
        const data = JSON.parse(JSON.stringify(row));
        this.dialogVisible = !this.dialogVisible;
        this.dialogType = {data, index};
      },
      tableRowStatus({row}) {
