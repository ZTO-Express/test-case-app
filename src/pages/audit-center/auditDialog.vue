  ...
  <div class="player-charge-dialog">
    <el-dialog
      ...
      <el-form :model="dialogForm" :rules="dialogForm1Rules" ref="dialogForm"
        ...
        <el-form-item prop="id" label="编号" v-if="false">
          <el-input v-model="dialogForm.id" v-bind:disabled="true">
          </el-input>
        </el-form-item>
...
<script>
  ...
  export default {
    ...
    computed: {
      price() {
        console.log(this.dialogForm.amount)
        console.log(this.dialogForm.classTotal)
    ...
    methods: {
      ...
      //   console.log(this.dialogForm.accountNo)
      // },
      initDialog() {
        this.dialogForm.accountNo = this.dialogForm.toAccountNo;
      ...
      async submit() {
        this.$refs.dialogForm.validate((pass) => {
          ...
          params.userId = this.$appData.userInfo.userId;
          params.toAccountNo = this.dialogForm.accountNo;
          params.transType = "charge";
          this.$axiosUtil.post(this.$appConfig.MIX, this.$urlConst.UPDATE_AUDIT_ORDER, params).then((res) => {
            if (res.code === 'S0001') {
              this.loading = !this.loading;
              this.close();
