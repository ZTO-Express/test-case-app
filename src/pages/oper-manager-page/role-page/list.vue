<template>
  <div align="left">
    <div align="right">
      <el-button size="small" type="primary" v-permission="'role-page-list/add'" icon="el-icon-plus"
                 @click="create()">
        新增角色
      </el-button>
    </div>
    ...
    <el-dialog :modal-append-to-body="false" class="role-create-dialog"
        ...
        <el-main>
          <el-form ref="dataForm" :model="temp" label-width="20%" size="mini" :rules="rules">
            ...
            <el-form-item label="角色状态" prop="status" align="left">
              <el-switch v-model="temp.status" active-text="启用" active-value="1"
                         inactive-value="0"></el-switch>
            </el-form-item>
            <el-form-item label="角色权限" align="left">
              <el-tree :check-on-click-node="true" :expand-on-click-node="false" ref="tree"
                       :data="menuTree"
    ...
    methods: {
      ...
      getTree() {
        ...
        if (this.$appData.userInfo.roles[0].systemRole === '1') {
          ...
          permAction.getMenuTreeByRole(this.$appData.userInfo.roles[0].roleCode)
            .then((rep) => {
              Object.assign(tempMenuTree, rep.data)
              // 去掉logo节点
              for (let i = 0; i < tempMenuTree.length; i += 1) {
              ...
              permAction.getUiTreeByRole(this.$appData.userInfo.roles[0].roleCode)
                .then((rep3) => {
      ...
      getList() {
        permAction.findRoles(this.$appData.userInfo.userId)
          .then((rep) => {
            this.list = rep.data
            this.sortList()
      ...
      edit(row) {
        ...
        permAction.getMenuTreeByRole(row.roleCode)
          .then((rsp) => {
            rsp.data.forEach((menu) => {
              this.getSelectMenus(menu, checkedMenuCode)
            })
            permAction.getPmsTreeByRoleCode(row.roleCode)
              .then((rsp1) => {
      ...
      save() {
        this.$refs.dataForm.validate((valid) => {
          ...
          if (valid) {
            ...
            permAction.updateRole(this.temp, this.dialogStatus, this.$appData.userInfo.userId)
              .then((response) => {
                ...
                permAction.delUiMetaByRoleCode(this.relatation.roleCode)
                  .then((rsp) => {
                    permAction.batchBandUiMeta(this.pmsRequest)
                      .catch((error1) => {
                        this.$message({
                          type: 'error',
                          message: error1.respMessage
                ...
                permAction.bandMenu(this.relatation)
                  .catch((error) => {
                    this.$message({
                      type: 'error',
                      message: error.respMessage
